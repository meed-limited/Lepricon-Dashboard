...
describe("LepriconStaking", function () {
    ...
    it("should stake 100 twice and emit events accordingly", async () => {
        ...
        await testToken.connect(admin).approve(lepriconStaking.address, amounts.xxl);

        // Add 1000 tokens to user1 as well
        await testToken.transfer(user1.address, amounts.l);
        // Approve and stake
        await testToken.connect(deployer).approve(lepriconStaking.address, amounts.l);
    ...
    it("should increase stakeholder's index with each new staker", async () => {
        ...
        const receipt = await lepriconStaking.connect(user1).stake(amounts.xs, time_lock);
    ...
    it("shouldn't be possible to stake more than the amount of tokens owned", async () => {
        ...
        await expect(lepriconStaking.connect(user2).stake(amountTooBig, 12)).to.be.revertedWith(
    ...
    it("shouldn't be possible to withdraw more than the current stake", async () => {
        ...
        await expect(lepriconStaking.connect(deployer).withdrawStake(amounts.s, 0)).to.be.revertedWith(
    ...
    it("should be possible to withdraw 50 from a 100 stake", async () => {
        ...
        expect(parseNumber(summary.total_amount)).to.equal(parseNumber(initialBalance) - parseNumber(withdraw_amount));
    ...
    it("should calculate the reward accordingly to APR and lockTime", async () => {
        ...
        await testToken.connect(deployer).approve(lepriconStaking.address, amounts.xl);
        ...
        expect(Number(newstake.claimable)).to.equal(1000 * 10 ** 18 * 0.0001096 * 91);
    ...
    it("should reward the stakes and transfer the tokens back to the user", async () => {
        ...
        expect(Number(second_balance)).to.equal(Number(after_balance) + 100 * 10 ** 18);
    ...
    it("should revert when setting NFT status", async () => {
        await expect(
            lepriconStaking.connect(user2).setNftStatus(user2.address, NFTtestAddress_BSC, 2, 2)
        ).to.be.revertedWith("Not authorized");

        await expect(
            lepriconStaking.connect(deployer).setNftStatus(user2.address, NFTtestAddress_BSC, 2, 25)
    ...
    it("should add an NFT booster and distribute reward accordingly", async () => {
        ...
        await lepriconStaking.connect(deployer).setNftStatus(user2.address, NFTtestAddress_BSC, 2, 2);
        ...
        expect((parseInt(stakeNFT.claimable.toString()) / 10 ** 18).toFixed(5)).to.equal(
            Number(100 * 0.0001096 * 182).toFixed(5)
        );

        // 6 months lock shouldn't include the NFT boost (=6%)
        expect(Number(stakeSixMonths.claimable)).to.equal(200 * 0.0001644 * 182 * 10 ** 18);

        await lepriconStaking.connect(user2).withdrawStake(amounts.xs, 0);

        await expect(lepriconStaking.connect(user3).resetNftStatus(user2.address)).to.be.revertedWith("Not authorized");
        ...
        const stakeSummaryAfter12Months = await lepriconStaking.hasStake(user2.address);
        const stakeTwelveMonths = stakeSummaryAfter12Months.stakes[2];
        expect((Number(stakeTwelveMonths.claimable) / 10 ** 18).toFixed(5)).to.equal(
            Number(200 * 0.0002192 * 364).toFixed(5)
        );
    });
    ...
    it("should edit the APR_NOLOCK successfully", async () => {
        ...
        await expect(lepriconStaking.connect(user2).setAPR(1, 0)).to.be.revertedWith(
